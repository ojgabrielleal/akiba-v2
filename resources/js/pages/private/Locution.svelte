<script>
    ...
    const redirectToDashboard = () => {
        router.get("/painel/dashboard/", {}, { preserveScroll: true });
    };
</script>
    ...
    <section transition:fade={{ duration: 500 }} class="fixed inset-0 flex items-center justify-center p-2 lg:p-0 z-50 bg-black/20 backdrop-blur-lg">
        <div class="w-sm p-6 rounded-xl bg-neutral-aurora">
            ...
            <div class="font-noto-sans mb-3">
                ...
                {onair.data.program.host.nickname} do ar? Segura ai! Antes deixa {onair.data.program.host.gender === "male" ? "ele" : "ela"} terminar o
                programa. Aqui ninguém sabe uma mágica milagrosa de passagem de microfone, então respeite o horário!
            </div>
            <button
...
{#if onair.data.type === "scheduled"}
    <section transition:fade={{ duration: 500 }} class="fixed inset-0 flex items-center justify-center p-2 lg:p-0 z-50 bg-black/20 backdrop-blur-lg">
        <div class="w-sm p-6 rounded-xl bg-neutral-aurora">
            ...
            <div class="font-noto-sans mb-3">
                Opa! Um programa agendado por um dos admininistradores já está no ar! Não tem como adiantar, pular ou cancelar na marra aqui ninguém
