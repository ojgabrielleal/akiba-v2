...
<Section title="Meus Programas">
    <form on:submit|preventDefault={submit}>
                ...
                {#each programs.data as item}
                    ...
                    >
                        <img
                            ...
                            aria-hidden="true"
                            loading="lazy"
                            class={["w-60 transition duration-300 ease-in-out", { "opacity-50 scale-90": $form.program === item.uuid }, { "opacity-100": $form.program !== item.uuid }, ]}
                        />
                    </button>
                {/each}
            </div>
        {/if}
        <div class="mb-8">
            <label for="phrase" class="text-orange-amber font-bold italic text-lg uppercase font-noto-sans block mb-1">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-9 gap-30 lg:gap-y-30 lg:gap-x-5 mt-28">
                {#each locutionIcons as item, index}
                    <button
                        ...
                        class={["cursor-pointer w-[9.55rem] h-12 flex justify-end items-end rounded-lg bg-suspense-aurora transition duration-300 ease-in-out", { "opacity-50 scale-90": $form.icon === item.url }, { "opacity-100": $form.icon !== item.url }, ]}
                        on:click={() => { $form.icon = item.url; }}
                    >
                        <img
                            src={item.url}
                            alt=""
        ...
        {#if can.start}
            <div class="flex justify-end">
                <button type="submit" class="cursor-pointer w-full lg:w-auto py-2 px-6 border-4 border-solid border-green-mint rounded-xl text-green-mint text-xl font-bold font-noto-sans italic uppercase">
                    Iniciar
                </button>
            </div>
        {/if}
