<script>
    ...
    const requestMarkTaskCompleted = (task) => {
        router.post(`/dashboard/task/${task}/complete`, {},
            { preserveScroll: true },
        );
    };
</script>
            ...
            {#if tasks.data.length > 0}
                {#each tasks.data as item}
                    <article class={["w-100 h-50 lg:w-160 lg:h-43 shrink-0 rounded-lg p-4 relative",
                        ...
                        {#if can.completed}
                            <button type="button" aria-label="Concluir tarefa" on:click={() => requestMarkTaskCompleted(item.uuid)} class={["font-noto-sans italic font-bold cursor-pointer",
