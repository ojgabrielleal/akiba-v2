<script>
    ...
    const submitVote = (event, item) => {
        ...
        router.post(`/panel/medias/poll/vote/${option}`, {},
            {
                preserveScroll: true,
                onSuccess: () => {
    ...
    const requestDeactivatePoll = (poll) => {
        router.delete(`/panel/medias/poll/${poll}`, {}, {
            preserveScroll: true,
        });
    };
</script>
    ...
    <Section title="Enquetes">
        {#if can.create || can.update}
            <div class="flex justify-center">
                <button
                    class="cursor-pointer text-neutral-aurora text-xl font-noto-sans font-bold uppercase italic rounded-sm py-1 px-3 bg-orange-amber"
                    on:click={() => {
                        offcanvasRef.open();
                        identifier = null;
                    }}
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 mt-10">
            {#if polls.data.length > 0}
                {#each polls.data as item}
                    {@const alreadyVoted = storageVotedPolls.includes(
                        item.uuid,
                    )}
                    <form
                    ...
                    >
                        <div
                            class="text-neutral-aurora text-xl text-start font-noto-sans font-bold"
                        >
                            {item.question}
                        </div>
                        <div class="flex flex-col gap-3">
                            {#each item.options as optitem}
                                <div class="inline-flex items-center">
                                    ...
                                    <label
                                        class="w-full ml-2 flex justify-between text-neutral-aurora text-md font-noto-sans cursor-pointer"
                                        for={optitem.id}
                                    >
                                        <div>
                                            {optitem.option}
                        ...
                        <div class="flex justify-between">
                            {#if can.vote.create}
                                <button
                                    type="submit"
                                    class="cursor-pointer text-neutral-aurora text-md font-noto-sans font-bold uppercase italic rounded-sm py-1 px-3 bg-orange-amber disabled:opacity-50"
                                    disabled={alreadyVoted}
                                >
                                    Votar
                                </button>
            ...
            {:else}
                <article
                    class="bg-blue-cerulean opacity-50 p-5 rounded-md pointer-events-none"
                >
                    <div
                        class="text-neutral-aurora text-xl text-start font-noto-sans font-bold mb-7"
                    >
                    ...
                    <div class="flex justify-between">
                        <button
                            type="submit"
                            class="cursor-pointer text-neutral-aurora text-md font-noto-sans font-bold uppercase italic rounded-sm py-1 px-3 bg-orange-amber"
                        >
                            Votar
                        </button>
                    </div>
