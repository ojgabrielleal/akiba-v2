<script>
    ...
    $: livePrograms = programs?.data?.filter((program) => program.type === "private") ?? [];
    $: scheduleItems = schedules?.data?.filter((item) => item.type === activeType) ?? [];
            ...
            {#each filters as filter}
                <button
                    ...
                    class={["cursor-pointer flex items-center gap-1 text-lg font-noto-sans font-extrabold italic uppercase hover:text-blue-skywave group/item", 
                        {"text-blue-skywave": activeType === filter.type},
                        {"text-neutral-gray": activeType !== filter.type}
                    ]}
                    on:click={() => activeType = filter.type}
                >
                    <img
                        src={filter.icon}
                        alt=""
                        class={["w-5 group-hover/item:filter-blue-skywave", 
                            {"filter-blue-skywave": activeType === filter.type},
                            {"filter-neutral-gray": activeType !== filter.type}
                        ]}
                        loading="lazy"
                    />
                    {filter.title}
                    ...
                    <article class="w-full">
                        <div>
                            ...
                            <div class="w-full h-13 flex items-center rounded-md px-3 bg-suspense-aurora relative mb-2">
                                ...
                                <div class="z-10 absolute bottom-2 right-22 bg-green-mint px-2 rounded-xl float-end text-center text-[0.6rem] text-suspense-aurora font-noto-sans font-extrabold italic uppercase">
                                    {activeType === "scheduled" ? "Gravado" : "Playlist"}
                                </div>
                                <img
                                    class="w-36 aspect-square absolute right-0 bottom-0 object-cover object-top"
                                    src={item.program.host.avatar}
                                    alt={item.program.host.nickname}
                                    loading="lazy"
                                />
                            </div>
                            <dl class="w-full rounded-md py-2 px-4 bg-suspense-aurora flex justify-between mb-2">
