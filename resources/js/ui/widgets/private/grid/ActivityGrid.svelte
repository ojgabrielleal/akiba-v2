    ...
    <Section {title}>
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
            {#each activities.data as item}
                <article
                    class="h-50 w-full shrink-0 rounded-lg p-4 relative bg-blue-skywave"
                >
                    <div
                        class="font-noto-sans font-black italic uppercase text-xl text-neutral-aurora"
                    >
                    ...
                    <div
                        class="flex gap-2 absolute bottom-3 left-4 items-center"
                    >
                        ...
                        {#if item.confirmations.length > 3}
                            <div class="relative group/popover">
                                ...
                                <div
                                    class="invisible group-hover/popover:visible opacity-0 group-hover/popover:opacity-100 translate-y-1 group-hover/popover:translate-y-0 transition-all duration-200 absolute bottom-full left-1/2 -translate-x-1/2 pb-3 z-10 pointer-events-auto"
                                >
                                    <div
                                        class="p-3 bg-neutral-900/95 backdrop-blur-md rounded-2xl shadow-2xl flex flex-col gap-2 items-center border border-white/10 max-h-80 overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-white/20 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-transparent"
                                    >
                                        {#each item.confirmations.slice(3) as rest}
                                            <span
                                                class="text-[11px] text-white/90 font-medium max-w-[80px] truncate text-center"
                                            >
                                                {rest.nickname}
                                            </span>
                                        {/each}
                                    ...
                                    <div
                                        class="absolute top-[calc(100%-12px)] left-1/2 -translate-x-1/2 border-4 border-transparent border-t-neutral-900/95"
                                    ></div>
                                </div>
                            </div>
                        {/if}
