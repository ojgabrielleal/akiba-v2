    ...
    <div class="h-100 py-3">
        ...
        <div class="text-sm font-noto-sans text-gray-500">
            Seu pedido já tá a caminho! {air.program.host.gender === "male"
                ? "O DJ"
                : "A DJ"}
            {air.program.host.nickname} vai ver rapidinho. Fica por aqui e curte
            a vibe da programação! ✨🔥
        </div>
    </div>
{:else if air.allows_song_requests}
    <form on:submit|preventDefault={submit}>
        ...
        <button
            type="submit"
            class="cursor-pointer w-full bg-blue-skywave px-8 py-2 rounded-md text-neutral-aurora font-noto-sans font-bold italic uppercase"
        >
            Enviar
        </button>
    </form>
    ...
    <div class="h-100 py-3">
        ...
        <div class="text-sm font-noto-sans text-gray-500">
            O programa não tá rolando ou {air.program.host.gender === "male"
                ? "o DJ"
                : "a DJ"}
            {air.program.host.nickname} tá dando uma pausa, tá? Mas relaxa, daqui
            a pouco você consegue mandar sua música! 💬🎶
        </div>
    </div>
{/if}
